...
Keep real database IDs in an ignored local registry or a secret manager. The tracked `references/databases.md` is only a safe template:
...
1. **Database IDs**: Add them to ignored local configuration or your secret manager
...
## Community Release Safety

Never commit a real Notion token, workspace ID, local-machine path, or personal contact detail. Before publishing, run:
