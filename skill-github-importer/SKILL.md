...
Use `read_url_content` or `browser_subagent` to retrieve the skills directory listing from GitHub.
...
Ask the user to select which skills to install.
...
Use `read_url_content` with GitHub API:
```
...
- Ask the user to confirm: "Security warnings detected. Do you want to proceed with installation?"
...
For approved skills, install to `~/.gemini/antigravity/skills/`:

1. Create the skill directory: `~/.gemini/antigravity/skills/{skill_name}/`
...
Use the `write_to_file` tool to create files.
...
**User:** "Install skills from https://github.com/example/antigravity-skills"
...
7. Installs approved skills to ~/.gemini/antigravity/skills/
