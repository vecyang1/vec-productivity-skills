...
2.  **Inject Principles (The Soul)**:
    *   Check for `## ⚖️ Principles` in the target `SKILL.md`.
    *   **If missing**: ASK the user (or extract from conversation history) for the core philosophy/principles of this skill.
    *   **Systemic Consistency (Inheritance)**:
        *   **Source of Truth**: Refer to the `## Principles` section at the bottom of **THIS** document (`skill-improver/SKILL.md`).
        *   **Action**: Select the universal principles (e.g., Traceability, Safety, Token Economy) from there and **APPEND** them to the target skill.
        *   **Format**:
        ```markdown
        ### 🛡️ Systemic Principles (Inherited)
        1. **Traceability**: ... (Copy from Skill Improver)
        2. **Safety First**: ... (Copy from Skill Improver)
